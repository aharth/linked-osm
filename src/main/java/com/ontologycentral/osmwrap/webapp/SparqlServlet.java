...
public class SparqlServlet extends HttpServlet {
    ...
    private void showSparqlForm(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        out.println("      <select id=\"example-select\">");
        out.println("        <option value=\"\">— select —</option>");
        out.println("        <option value=\"q1\">Count per class</option>");
        out.println("        <option value=\"q2\">Named features</option>");
        out.println("        <option value=\"q3\">Features with coordinates</option>");
        out.println("        <option value=\"q4\">Contributors</option>");
        out.println("        <option value=\"q5\">CONSTRUCT geometry subgraph</option>");
        out.println("      </select>");
        out.println("    </div>");
        out.println("    <div>");
        out.println("    <textarea cols=\"100\" rows=\"20\" name=\"query\">PREFIX rdf: &lt;http://www.w3.org/1999/02/22-rdf-syntax-ns#&gt;");
        out.println("");
        out.println("SELECT ?class (COUNT(?s) AS ?count)");
        out.println("FROM &lt;/map.rdf?bbox=11.075,49.457,11.077,49.459&gt;");
        out.println("WHERE { ?s a ?class }");
        out.println("GROUP BY ?class");
        out.println("ORDER BY DESC(?count)</textarea>");
        out.println("    </div>");
        out.println("    <div>");
        ...
        out.println("  </form>");
        out.println("  <script>");
        out.println("  var EXAMPLES = {");
        out.println("    q1: \"PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>\\n\\nSELECT ?class (COUNT(?s) AS ?count)\\nFROM </map.rdf?bbox=11.075,49.457,11.077,49.459>\\nWHERE { ?s a ?class }\\nGROUP BY ?class\\nORDER BY DESC(?count)\",");
        out.println("    q2: \"PREFIX rdf:  <http://www.w3.org/1999/02/22-rdf-syntax-ns#>\\nPREFIX dc:   <http://purl.org/dc/elements/1.1/>\\n\\nSELECT ?s ?name\\nFROM </map.rdf?bbox=11.075,49.457,11.077,49.459>\\nWHERE {\\n  ?s a <http://geovocab.org/spatial#Feature> ;\\n     dc:title ?name .\\n}\\nORDER BY ?name\",");
        out.println("    q3: \"PREFIX geo: <http://www.w3.org/2003/01/geo/wgs84_pos#>\\nPREFIX dc:  <http://purl.org/dc/elements/1.1/>\\n\\nSELECT ?s ?lat ?lon ?name\\nFROM </map.rdf?bbox=11.075,49.457,11.077,49.459>\\nWHERE {\\n  ?s geo:lat ?lat ;\\n     geo:long ?lon .\\n  OPTIONAL { ?s dc:title ?name }\\n}\\nORDER BY ?name\",");
        out.println("    q4: \"PREFIX prov: <http://www.w3.org/ns/prov#>\\nPREFIX foaf: <http://xmlns.com/foaf/0.1/>\\n\\nSELECT ?user (COUNT(?s) AS ?edits)\\nFROM </map.rdf?bbox=11.075,49.457,11.077,49.459>\\nWHERE {\\n  ?s prov:wasAttributedTo ?agent .\\n  ?agent foaf:nick ?user .\\n}\\nGROUP BY ?user\\nORDER BY DESC(?edits)\",");
        out.println("    q5: \"PREFIX geo: <http://www.w3.org/2003/01/geo/wgs84_pos#>\\nPREFIX dc:  <http://purl.org/dc/elements/1.1/>\\n\\nCONSTRUCT { ?s geo:lat ?lat ; geo:long ?lon ; dc:title ?name }\\nFROM </map.rdf?bbox=11.075,49.457,11.077,49.459>\\nWHERE {\\n  ?s geo:lat ?lat ;\\n     geo:long ?lon .\\n  OPTIONAL { ?s dc:title ?name }\\n}\"");
        out.println("  };");
        out.println("  document.getElementById('example-select').addEventListener('change', function() {");
        out.println("    if (!this.value) return;");
        out.println("    var ta = document.querySelector('textarea[name=\"query\"]');");
        out.println("    var m = ta.value.match(/FROM\\s+<([^>]*)>/i);");
        out.println("    var currentFrom = m ? m[1] : '/map.rdf?bbox=11.075,49.457,11.077,49.459';");
        out.println("    ta.value = EXAMPLES[this.value].replace(/FROM <[^>]*>/, 'FROM <' + currentFrom + '>');");
        out.println("  });");
        out.println("  window.addEventListener('DOMContentLoaded', function() {");
        out.println("    var hash = window.location.hash;");
        out.println("    if (!hash) return;");
        out.println("    var hm = hash.match(/[#&]from=([^&]+)/);");
        out.println("    if (!hm) return;");
        out.println("    var from = decodeURIComponent(hm[1]);");
        out.println("    var ta = document.querySelector('textarea[name=\"query\"]');");
        out.println("    if (!ta) return;");
        out.println("    ta.value = ta.value.replace(/FROM\\s+<[^>]+>/, 'FROM <' + from + '>');");
        out.println("  });");
        out.println("  </script>");
        out.println("</body>");
        out.println("</html>");
