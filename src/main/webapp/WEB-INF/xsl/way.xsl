<xsl:stylesheet
  ...
  <xsl:template match="osm">
    ...
    <xsl:text>    dc:date "</xsl:text><xsl:value-of select="way/@timestamp"/><xsl:text>" ;&#10;</xsl:text>
    ...
    <xsl:text>    rdfs:label "OSM XML to RDF Way Transformation" ;&#10;</xsl:text>
    <xsl:text>    prov:used &lt;</xsl:text><xsl:value-of select="$source-prefix"/><xsl:text>/way/</xsl:text><xsl:value-of select="way/@id"/><xsl:text>&gt; ;&#10;</xsl:text>
    ...
    <xsl:if test="$upstream-bytes >= 0">
      <xsl:text>&lt;</xsl:text><xsl:value-of select="$source-prefix"/><xsl:text>/way/</xsl:text><xsl:value-of select="way/@id"/><xsl:text>&gt; dcat:byteSize "</xsl:text><xsl:value-of select="$upstream-bytes"/><xsl:text>"^^xsd:decimal .&#10;</xsl:text>
      <xsl:text>&#10;</xsl:text>
    </xsl:if>
