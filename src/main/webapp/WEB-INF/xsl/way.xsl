  ...
  <xsl:template match="osm">
    ...
    <xsl:call-template name="doc-header">
      <xsl:with-param name="primary-source-url">
        <xsl:choose>
          <xsl:when test="way/@version">
            <xsl:text>https://api.openstreetmap.org/api/0.6/way/</xsl:text><xsl:value-of select="way/@id"/><xsl:text>/</xsl:text><xsl:value-of select="way/@version"/>
          </xsl:when>
  ...
  <xsl:template match="way">
    ...
    <xsl:if test="@version">
      <xsl:call-template name="versioned-source">
        <xsl:with-param name="type">way</xsl:with-param>
